...
async function askLlm(mandate, goal) {
...
async function propose(goal) {
  ...
  const mandate = { agent, approvedRecipient, maxTransferPlumbus: ethers.formatUnits(maxTransferAmount, 18) };
  const rawProposal = await askLlm(mandate, goal);
